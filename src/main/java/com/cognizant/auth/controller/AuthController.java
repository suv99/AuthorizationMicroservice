...
public class AuthController {
	...
	public ResponseEntity<AuthResponse> getValidity(@RequestHeader("Authorization") String token) {
		...
		if (jwtutil.validateToken(token.substring(7))) {
			res.setUid(jwtutil.extractUsername(token.substring(7)));
			res.setValid(true);
			res.setName((userRepository.findById(jwtutil.extractUsername(token.substring(7))).orElse(null).getUname()));
